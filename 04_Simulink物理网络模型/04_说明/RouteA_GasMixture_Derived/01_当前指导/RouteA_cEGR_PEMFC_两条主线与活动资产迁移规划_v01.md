...
文件类型：当前活动仓总规划与资产边界
日期：2026-08-17
状态：活动仓初始化完成；六配置矩阵为后续研究目标，现有结果按证据状态保留，不把未实现架构写成已完成。
...
完整系统主线优先保证平台可复用性；聚焦主线不得复制完整系统 runner 来表达工况，也不得用尚未闭合的设备代理替代真实设备物理结论。
...
1. 完成完整系统面板的入口、依赖、参数注册、观测注册和单工况回归收口；
2. 固化聚焦模型统一 case/runner/结果契约，先完成 `V-SH` 和 `V-MH` 的可比基线；
3. 修复或替换 FuelCell 域引射器本构，先完成 E-SH 组件级与关闭/开启边界验证；
4. 在设备参数合同和能量代价明确后建立主动泵配置；
5. 再建立 E-MH、P-SH、P-MH，并用同一工况矩阵进行工程比较；
6. 每个配置完成结构读回、`model_check`、最小 smoke、代表性行为验证后，才进入参数匹配和 240 kW 场景研究。
